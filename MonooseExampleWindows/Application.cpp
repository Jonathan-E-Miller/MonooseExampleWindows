...
static int check_pass(const char* user, const char* pass) {
	(void)user;
	return (strcmp(pass, "testpassword123") == 0);
}
...
static void logout_handler(struct mg_connection* nc, int ev, void* p) {
	...
	if (s != NULL) {
		fprintf(stderr, "%s logged out, session %" INT64_X_FMT " destroyed\n",
			s->user, s->id);
		destroy_session(s);
		fprintf(stderr, "Session has been destroyed");
	}
	nc->flags |= MG_F_SEND_AND_CLOSE;
	(void)ev;
}
...
*
 */
static void test_handler(struct mg_connection* nc, int ev, void* p) {
	struct http_message* hm = (struct http_message*) p;
	...
	if (s == NULL) {
		...
		char shead[100];
		snprintf(shead, sizeof(shead),
			"Set-Cookie: %s=%" INT64_X_FMT "; path=/", SESSION_COOKIE_NAME, s->id);
		/*
...
int main(void) {
	...
	mg_register_http_endpoint(nc, "/test.html", test_handler);
	mg_set_timer(nc, mg_time() + SESSION_CHECK_INTERVAL);
